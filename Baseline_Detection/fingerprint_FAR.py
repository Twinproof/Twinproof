...
SCORE_COLUMN = "Fingerprint_Score"


def parse_args():
    project_root = Path(__file__).resolve().parents[1]
    default_input = project_root / "Claim_Detection" / "results" / "fingerprint"
    default_output = Path(__file__).resolve().parent / "fingerprint_far_results.csv"

    parser = argparse.ArgumentParser(
        description="Calculate FAR for fingerprint scores under multiple thresholds."
    )
    parser.add_argument(
        "--input-dir",
        type=Path,
